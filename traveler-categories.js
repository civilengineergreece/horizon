...
function installPlannerPatch(){
  ...
  fieldsHTML=function(step){
    ...
    return `<div class="fields traveler-age-grid">
      <div class="field"><label>Ταξιδιώτες 12+ ετών</label><input class="control" id="adults" type="number" min="1" max="30" value="${adults}"></div>
      <div class="field"><label>Παιδιά 2–11 ετών</label><input class="control" id="children" type="number" min="0" max="30" value="${children}"></div>
      <div class="field"><label>Βρέφη κάτω των 2 ετών</label><input class="control" id="infants" type="number" min="0" max="30" value="${infants}"></div>
    </div><div class="traveler-age-note">Οι κατηγορίες ηλικίας βοηθούν το Horizon να υπολογίζει καλύτερα πτήσεις, διαμονή και συνολικό budget. Ορισμένες αεροπορικές υπηρεσίες έχουν δικά τους όρια ανά online αναζήτηση.</div>`;
  };
  ...
  capture=function(){
    ...
    state[s.key]={
      adults:Math.max(1,n(a?.value,1)),
      children:Math.max(0,n(c?.value,0)),
      infants:Math.max(0,n(i?.value,0))
    };
  ...
  if(typeof validate==='function'){
    ...
    validate=function(){
      ...
      const t=travelers();
      return t.adults>=1&&t.children>=0&&t.infants>=0;
    };
  }
  ...
  calcCost=function(d){
    ...
    let stayFactor=.94;
    if(state.stay==='hotel')stayFactor=1.12;
    else if(state.stay==='airbnb')stayFactor=.98;
    else if(state.stay==='camping')stayFactor=.66;
    const accommodation=Math.round(d.daily*nights*weighted*.52*stayFactor);
    const foodLocal=Math.round(d.daily*days*(t.adults+t.children*.55+t.infants*.08)*.34);
    const activities=Math.round(d.activity*days*(t.adults+t.children*.55+t.infants*.05));
    const transport=state.budget?.transport==='no'?0:Math.round(d.travel*(t.adults+t.children*.7+t.infants*.15));
