...
function collect(editor){const name=editor.dataset.destination||document.querySelector('.horizon-detail-overlay .hd-head h3')?.textContent?.trim()||'Ταξίδι';const days=[...editor.querySelectorAll('.hz-edit-day')].map((day,i)=>({day:i+1,title:safe(day.querySelector('.hz-day-title')?.value)||`Ημέρα ${i+1}`,rows:[...day.querySelectorAll('.hz-edit-row')].map(r=>({time:safe(r.querySelector('.hz-slot-time')?.value),text:safe(r.querySelector('.hz-slot-text')?.value)})).filter(x=>x.time||x.text),notes:day.querySelector('.hz-edit-notes')?.value||''}));return {name,days,tripNotes:editor.querySelector('.hz-trip-notes')?.value||''};}
...
async function makePdf(data){
  ...
  const addHeader=(page,title,sub='')=>{page.drawRectangle({x:0,y:775,width:A4[0],height:67,color:rgb(.035,.10,.15)});page.drawText('HORIZON',{x:38,y:808,size:17,font,color:rgb(1,.48,.09)});page.drawText(title,{x:38,y:786,size:12.5,font,color:rgb(.96,.98,1)});if(sub)page.drawText(sub,{x:38,y:762,size:8.5,font,color:rgb(.38,.48,.55)});};
  ...
  const summary=pdf.addPage(A4);addHeader(summary,`${data.name} · Προσωπικό itinerary`,`${data.days.length} ημέρες · διαδραστικό PDF`);
  ...
  const trip=form.createTextField('trip_notes');prepareMultiline(trip);trip.setText(data.tripNotes||'');trip.addToPage(summary,fieldOpts({x:38,y:410,width:519,height:245}));finishField(trip,font,10);
  ...
  data.days.forEach(day=>{
    ...
    const title=form.createTextField(`day_${day.day}_title`);title.setText(day.title);title.addToPage(page,fieldOpts({x:38,y:680,width:519,height:30}));finishField(title,font,11);
    page.drawText('Ολοκληρώθηκε',{x:58,y:651,size:9,font,color:rgb(.08,.12,.16)});const cb=form.createCheckBox(`day_${day.day}_done`);cb.addToPage(page,{x:38,y:646,width:14,height:14,borderWidth:1,borderColor:rgb(.35,.4,.45)});
    page.drawText('Πρόγραμμα ημέρας — μπορείς να αλλάξεις ώρες και δραστηριότητες',{x:38,y:620,size:9,font,color:rgb(.08,.12,.16)});
    const plan=form.createTextField(`day_${day.day}_plan`);prepareMultiline(plan);plan.setText(day.rows.map(r=>`${r.time}${r.time&&r.text?'  ':''}${r.text}`).join('\n'));plan.addToPage(page,fieldOpts({x:38,y:300,width:519,height:300}));finishField(plan,font,10);
    page.drawText('Προσωπικές σημειώσεις ημέρας',{x:38,y:274,size:9,font,color:rgb(.08,.12,.16)});
    const notes=form.createTextField(`day_${day.day}_notes`);prepareMultiline(notes);notes.setText(day.notes||'');notes.addToPage(page,fieldOpts({x:38,y:90,width:519,height:165}));finishField(notes,font,10);
  });
...
function install(){burst();document.addEventListener('click',e=>{const btn=e.target.closest?.('[data-download-pdf-fixed]');if(!btn){burst();return;}e.preventDefault();e.stopImmediatePropagation();const editor=btn.closest('.hz-itinerary-editor');if(!editor)return;(async()=>{try{btn.disabled=true;status(editor,'Δημιουργία διαδραστικού PDF…');const data=collect(editor);const bytes=await makePdf(data);download(bytes,data.name);status(editor,'Το διαδραστικό PDF δημιουργήθηκε και κατέβηκε.',true);}catch(err){console.error('Horizon PDF',err);status(editor,`Η δημιουργία PDF απέτυχε (${err?.message||'άγνωστο σφάλμα'}).`);}finally{btn.disabled=false;}})();},true);}
